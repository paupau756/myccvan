            ...
            <table class="receipt-table">
                ...
                <tbody>
                    ...
                    if ($driverResult->num_rows > 0) {
                        ...
                        <tr>
                            <td colspan="2">
                                <form action="changedriver.php" method="POST">
                                    <input type="hidden" name="inquiryid" value="<?php echo $row['inquiryid']; ?>">
                                    <label for="newDriver">Select New Driver:</label>
                                    <select name="newDriver" id="newDriver">
                                        <!-- Populate select options with available drivers -->
                                        <?php
                                        $allDriversQuery = "SELECT * FROM drivers";
                                        $allDriversResult = $conn->query($allDriversQuery);
                                        if ($allDriversResult->num_rows > 0) {
                                            while ($driver = $allDriversResult->fetch_assoc()) {
                                                // Check if the current driver matches the option, and mark it as selected
                                                $selected = ($driver['driverid'] == $row['driverid']) ? "selected" : "";
                                                echo "<option value='{$driver['driverid']}' $selected>{$driver['name']}</option>";
                                            }
                                        }
                                        ?>
                                    </select>
